...
{
  void confirmPassword(const std::shared_ptr < WalletBackend > walletBackend,
  ...
  std::tuple < uint64_t, uint64_t > getScanRange() 
  {
    ...
    while (true) 
    {
      ...
      std::cout << "\n\n" << InformationMsg("What height would you like to end ") <<
        ...
        InformationMsg(defaultEndHeight) <<
        InformationMsg(": ");
      ...
      if (stringEndHeight == "") 
      {
        ...
        stringEndHeight = std::to_string(std::stoull(stringStartHeight) + 1000);
      }

      try {
        return 
        {
          std::stoull(stringStartHeight),
          std::stoull(stringEndHeight)
        };
      } 
      catch (const std::out_of_range & ) 
      {
